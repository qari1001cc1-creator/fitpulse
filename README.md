...
> AI keys are copied from `SEO-AGENT\.env`. Voice uses the **browser Web Speech API â€” free, no key**.

## ðŸ§© Tech Stack (all free)

- **Backend:** Python Flask + SQLite (multi-user, werkzeug hashed passwords)
- **Frontend:** Jinja2 + custom dark CSS + Chart.js (CDN)
- **AI:** OpenRouter â†’ Groq chain (free models)
...
- **APK:** see `apk/` folder â€” Android WebView wrapper project

## âœ¨ Features

- **Daily Exercise Plan** â­ auto-generated from goal/experience/days + video demos
...
templates/        all pages
static/           css, voice-v2.js, manifest, service-worker
apk/              Android WebView wrapper
```

## ðŸ“± Phone Demo

1. Find your PC's LAN IP: `ipconfig` â†’ IPv4 (e.g. `192.168.1.5`)
2. Open `http://192.168.1.5:8520` on the phone (same Wi-Fi)
3. On Android: browser menu â†’ **Add to Home screen** â†’ installs as app (PWA)

## ðŸ“± Android APK

See `apk/README.md`. Open the `apk/FitPulse/` folder in **Android Studio**, set the URL to your LAN IP or a hosted URL, and Build â†’ APK. No Play Store required.
